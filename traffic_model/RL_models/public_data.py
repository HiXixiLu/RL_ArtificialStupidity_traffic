...
NORM_NON_SD = MAX_BICYCLE_AC / 2.576    # 正态分布的标准差σ：满足 X~N(0, σ^2) 使上α分位点在 0.2 * MAX_BICYCLE_AC，其中α=0.005
HUMAN_SD = (EXPLORATION_NOISE * MAX_HUMAN_VEL) / 2.576  # 人类速度标准差

MAX_EPISODE =  40000 # num of games —— 进行迭代的次数，但其中有一部分无效计算时间是用于填满样本池的
# MAX_TRAINING_EPOCH = 2000  # num of traning batch: 实际训练次数
MAX_LENGTH_OF_TRAJECTORY = 200    # num of frames —— 单次训练最大序列长度
PRINT_LOG = 5
UPDATE_ITERATION = 50   # 一次网络参数更新的均值计算次数

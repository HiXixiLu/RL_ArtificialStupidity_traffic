...
# 一个 DDPG_Straight 实例对应一个 agent
# TODO：完成与自己项目相关的修改
class DDPG_Straight(object):
    device = 'cpu'
    # state_dim : 状态维度
    # action_dim： 动作维度
    # max_action：动作限制向量
    ...
    def update(self):
        ...
        for it in range(pdata.UPDATE_ITERATION):
            ...
            pdata.write_to_log('critic_loss:{loss}'.format(loss=critic_loss))
            ...
            pdata.write_to_log('actor_loss:{loss}'.format(loss=actor_loss))
    ...
    def save(self):
        torch.save(self.actor.state_dict(), pdata.DIRECTORY + 'actor_straight.pth')
        torch.save(self.critic.state_dict(), pdata.DIRECTORY + 'critic_stright.pth')
        print("====================================")
        print("Model has been saved...")
        print("====================================")

    def load(self):
        self.actor.load_state_dict(torch.load(pdata.DIRECTORY + 'actor_straight.pth'))
        self.critic.load_state_dict(torch.load(pdata.DIRECTORY + 'critic_stright.pth'))
        print("====================================")
